  ...
  return (
    <div className="font-comfortaa w-[90%] h-screen m-auto">
      ...
      <div className="columns-2 text-center">
        {!currentOpening
          ? null
          : currentOpening.continuations.map((continuation) => {
              const continuations2 = fenToPosition(
                continuation.fen
              ).continuations;
              return (
                <motion.div
                  ...
                  <motion.div
                    ...
                    initial={{
                      opacity: 0,
                      x: randOffset() / 2,
                      transition: { delay: (randOffset() + 25) / 70 },
                    }}
                    animate={{ opacity: 1 }}>
                    <motion.div
                      ...
                      {...buttonStyles}
                      onClick={() => {
                        handleGen1Click(continuation.move);
                      }}
                      onHoverStart={() => {
                        handleHover(continuation.move);
                      }}
                      onHoverEnd={() => {
                        handleHover(null);
                      }}
                      ...
                      <h1 className="text-md md:text-md lg:text-lg text-accent w-full">
                        {continuation.move}
                      </h1>
                      <h1 className="text-md md:text-lg lg:text-xl">
                        {continuation.positionName}
                      </h1>
                    </motion.div>
                  </motion.div>
                      ...
                      return (
                        <motion.div
                          ...
                          animate={{
                            opacity: 1,
                            transition: {
                              delay: index2 / (continuations2.length / 1.5),
                            },
                          }}>
                          <motion.div
                            ...
                            <p className="text-xs lg:text-md text-accent w-full">
                              {continuation.move} + {continuation2.move}
                            </p>
                            <p className="text-xs lg:text-md">
                              {continuation2.positionName}
                            </p>
                          </motion.div>
                        </motion.div>
                      );
